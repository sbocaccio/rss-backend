...
class SubscriptionFeedAPI(ListCreateAPIView):
    ...
    def delete(self, request):
        subscription = self.check_user_is_subscribed_to_subscription(request.data['subscription_id'], request.user)
        self.delete_user_articles_from_user(request)
        subscription.users_subscribed.remove(request.user)
        if (len(subscription.users_subscribed.all()) == 0):
            subscription.delete()
        return Response({
